...
<Haiku
  ...
  lines={[
    {
      text: "A model swapped out",
      why: "The morning opened on the \"tell a fable\" thread — Fable 5 pulled and restored, with BridgeMind claiming the brought-back build came back quietly nerfed and silently falling back to Opus 4.8. It rhymed all day: MoE routing you never see, OpenRouter picking providers for you. When the model behind the API can change under you with no version bump, a headline benchmark is worth what, exactly?",
    },
    {
      text: "agents paying by the call",
      why: "The day's agent-economy arc — x402 reviving HTTP 402 so an agent signs a gasless USDC transfer and pays per request with no key and no human, Alipay's Token Pay doing the walled-garden version, AP2 wrapping it in signed mandates. Software holds the wallet now and spends on its own.",
    },
    {
      text: "who pays when it breaks?",
      why: "Where the payment rails landed — AP2's own open question of who eats the loss when a prompt-injected agent produces a perfectly valid mandate, running straight into Worldcoin gating the humans out and the UBI / Idiocracy coda about who catches the people the curve leaves behind. Payment required, and the bill isn't only in USDC.",
    },
  ]}
/>
